...
"""에러바는 데이터의 표준 편차를 나타내는 경우에는 길이가 길 수록 해당 그룹의 데이터가 퍼져있음을 의미"""

#에러바로 요그므이 평균과 표준 편차 표현
plt.figure(figsize=(10,6))
...
## **타이타닉 데이터셋으로 개별 서브 플롯 하나씩 그리기**

#Survived 가 0이면 사망자를,1이면 생존자를 나나탬
titanic = pd.read_csv('3.1.1.titanic.csv')

# 각 부ㅇ무와 자녀의 수에 따른 생존자와 사망자 수 계산
...
# 두 번째 서브플롯 설정(막대 그래프)
axes[1].bar(x,y2,'-o',color='deeppink',alpha = 0.7, width = 0.5,label = 'Survived')
axes[0].set_xlabel('Parch')
axes[0].set_ylabel('Survived Count',color = 'deeppink')
axes[0].tick_params(axis = 'y', labelcolor='deeppink')
axes[0].legend(loc='upper right')
...
def main():
    ...
    jsonResult, result, natName, dataEND = getTourusmStatsService(nat_cd, ed_cd, nStartYear, eEndYear) #[CODE 3]
...
def getTourusmStatsService(nat_cd, ed_cd, nStartYear, nEndYear):
    ...
    for year in range(nStartYear, nEndYear+1):
        for month in range(1, 13):
            ...
            jsonData = getTourusmStatsItem(yyyymm, nat_cd, ed_cd)   #[CODE 2]
            if (jsonData['reponse']['header']['resultMsg'] == 'OK'):
                ...
                natName = jsonData['reponse']['body']['items']['item']['natKorNm']
                natName = natName.replace(' ','')
                num = jsonData['reponse']['body']['items']['item']['num']
                ed = jsonData['reponse']['body']['items']['item']['ed']
                print('[ %s_%s : %s]' % (natName, yyyymm, num))
                print('------------------------------------------------------')
                jsonResult.append([natName, nat_cd, yyyymm, num])

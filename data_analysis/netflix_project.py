...
# 데이터 프레임의 각 컬럼별 결측치 개수 반환
# isna() == isnull(): 결측값은 True 반환하고, 그 외에는 False 반환
netflix.isna().sum()
...
# title열의 값이 'Sankofa'인 행 전체를 확인하여 country 열과 age_group 열의 값이 어떻게 이루어져 있는 지 확인
netflix_age_country[netflix_age_country['title'].str.contains('Sankofa', na=False, case=False)]

# 각 나이 그룹에 다른 국가별 넷플릭스 콘텐츠 수 구하기
...
# mask : 단어를 그릴 위치 설정, 흰샌(#FFFFFF) 항목은 마스킹된 것으로 간주
# 로고 이미지 열고 넘파이 배열로 변환
mask = np.array(Image.open('netflix_logo.jpg'))

# 워드 클라우드 색상맵 만들기
cmap = plt.matplotlib.colors.L